...
function diciannove(){     
   ...
   if (window.matchMedia("(max-width: 1200px)").matches)
       {                
        // verifico alta densità
         if ((window.screen.width)>(window.innerWidth)) // bassa densità (firefox tablet landscape)
             window.open('primoquadro.htm','','width='+screen.availWidth+',height='+screen.availHeight+'');
          else if (((window.innerWidth)<(window.screen.width)*2)&&(window.matchMedia("(orientation: portrait)").matches))
		          {			  
                    // portrait tablet bassa densita'
				    window.open('primoquadro.htm','','width='+screen.availWidth+',height='+screen.availHeight+'');
				  }
                else 
